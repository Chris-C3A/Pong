...
bool restart(int pl_score, int comp_score) {
    system("cls");
    ...
    if (pl_score > comp_score) {
        ...
        cout << "You lost! Would you like to try again?(Y/N)\n";
    }
    char re;
    bool ret;
    cin >> re;
...
int main()  {
  ...
  while ( true ) {
    if ( started ) {
      ...
      if (you_score == 10 || comp_score == 10) {
          bool ret = restart(you_score, comp_score);
          if (!ret) {
              break;
